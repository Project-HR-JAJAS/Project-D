...
@router.get("/api/all-authentication-ids-with-specific-fraud/{reason}")
def get_all_authentication_ids_with_specific_fraud(reason: str):
    try:
        data = find_unique_authentication_ids_with_specific_fraud(reason)
        return data
    except Exception as e:
        print(f"Error: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
def find_unique_authentication_ids_with_specific_fraud(reason):
    db = DbContext()
    db.connect()
    cursor = db.connection.cursor()
